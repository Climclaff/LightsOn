...
namespace LightOn.Controllers
{
    ...
    public class StreetController : ControllerBase
    {
        ...
        {
            ServiceResponse<Street> result = await _service.DeleteAsync(id);
            if (result.NotFound)
            {
                return NotFound();
            }
        ...
        {
            ServiceResponse<Street> result = await _service.CreateAsync(street);
            if (result.Success)
            {
                return Ok();
            }
        ...
        [HttpPost]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] Street street)
        {
            ServiceResponse<Street> result = await _service.UpdateAsync(street);
            if (result.NotFound)
            {
                return NotFound();
            }
